...
执行`buaa-doc\sql\tx-manager.sql`和`buaa-doc\sql\tx_logger.sql`脚本

### 2. 修改TxManager的配置

工程目录：`buaa-transaction\txlcn-tm`，修改nacos、数据库和redis

### 2. 启动TxManager

工程目录：`buaa-transaction\txlcn-tm`

### 3. 初始化demo的数据

执行`buaa-demo\txlcn-demo\txlcn-demo.sql`脚本
